...
@torch.no_grad()  #Context manager to tell pytorch not to backtrack.
def estimate_loss():
    out = {}
    model.eval()
    for split in ['train', 'val']:
        ...
        for k in range(eval_iters):
            ...
            losses[k] = loss.item()
        out[split] = losses.mean()   #Average loss over all batches, to reduce noise
    model.train()
    return out
...
class BigramLanguageModel(nn.Module):
    ...
    def __init__(self):
        ...
        self.blocks = nn.Sequential(*[Block(n_embd, n_head=n_head) for _ in range(n_layer)]) # 3 blocks of 4 heads of 8-dimensional self-attention
        ...
        #self.sa_head = MultiHeadAttention(n_heads=4, head_size=n_embd // 4) # 4 heads of 8-dimensional self-attention
        #self.ffwd = FeedForward(n_embd)
